...
void Scheduler::unbind()
{
    ...
    std::unique_ptr<Worker> worker;
    {
        ...
        MARL_ASSERT(it != bound->singleThreadedWorkers.end(), "singleThreadedWorker not found");
...
Scheduler::~Scheduler()
{
    {
        std::unique_lock<std::mutex> lock(singleThreadedWorkerMutex);
        MARL_ASSERT(singleThreadedWorkers.size() == 0, "Scheduler still bound on %d threads", int(singleThreadedWorkers.size()));
    }
...
void Scheduler::enqueue(Task&& task)
{
    ...
    else
    {
        ...
        MARL_ASSERT(it != singleThreadedWorkers.end(), "singleThreadedWorker not found");
...
{
    MARL_ASSERT(currentFiber == from, "Attempting to call yield from a non-current fiber");
...
{
    MARL_ASSERT(mode == Mode::SingleThreaded, "flush() can only be used on a single-threaded worker");
    std::unique_lock<std::mutex> lock(work.mutex);
    runUntilIdle(lock);
}
...
{
    MARL_ASSERT(work.num == work.fibers.size() + work.tasks.size(), "work.num out of sync");
    if (work.num == 0)
    {
