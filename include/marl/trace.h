...
#define MARL_SCOPED_EVENT(...) marl::Trace::ScopedEvent MARL_CONCAT(scoped_event, __LINE__)(__VA_ARGS__);
#define MARL_BEGIN_ASYNC_EVENT(id, ...) do { if (auto t = marl::Trace::get()) { t->beginAsyncEvent(id, __VA_ARGS__); } } while(false);
#define MARL_END_ASYNC_EVENT(id, ...) do { if (auto t = marl::Trace::get()) { t->endAsyncEvent(id, __VA_ARGS__); } } while(false);
#define MARL_SCOPED_ASYNC_EVENT(id, ...) marl::Trace::ScopedAsyncEvent MARL_CONCAT(defer_, __LINE__)(id, __VA_ARGS__);
#define MARL_NAME_THREAD(...) do { if (auto t = marl::Trace::get()) { t->nameThread(__VA_ARGS__); } } while(false);
